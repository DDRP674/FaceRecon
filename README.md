...
为每张图生成 0% 到 90% 十档高斯噪声图，并用 insightface `buffalo_l` 提取 embedding。缓存按噪声档位和 shard 保存在 `work/embeddings/noise_XX/`，已存在的 shard 默认跳过。

```bash
python run_pipeline.py stage0 --batch-size 64 --ctx-id 0 --save-every 4
```

如果要缓存 SDXL VAE latent，建议单独先跑 VAE，这样 12 小时作业不会因为多噪声 embedding 太慢而还没轮到 VAE：
...
默认用测试集做 query、训练集做 gallery，报告 top1/top5。
